...
instance (ConvertableBuiltin builtin1 builtin2) => Quote (Thunk builtin1) (Expr builtin2) where
  quote p level = \case
...
instance (ConvertableBuiltin builtin1 builtin2) => Quote (ForcedValue builtin1) (Expr builtin2) where
  quote p level = \case
    F.VUniverse u -> Universe p u
    F.VMeta m spine -> quoteApp level p (Meta p m) spine
    ...
    F.VPi binder closure -> do
      let quotedBinder = quote p level binder
      let quotedBody = quoteForcedClosure p level (binder, closure)
      Pi p quotedBinder quotedBody
...
quoteForcedCtx :: (ConvertableBuiltin builtin1 builtin2) => Provenance -> Lv -> F.BoundEnv builtin1 -> Substitution (Expr builtin2)
quoteForcedCtx p level env i = Right (quote p level (F.lookupIxInEnv env i))

quoteForcedClosure :: (ConvertableBuiltin builtin1 builtin2) => Provenance -> Lv -> (GenericBinder expr, F.Closure builtin1) -> Expr builtin2
quoteForcedClosure p lv (binder, F.Closure env body) = do
  -- Here we deliberately avoid using the standard `quote . eval` approach below
  -- on the body of the lambda, in order to avoid the dependency cycles that
  -- prevent us from printing during NBE.
  --

...
pattern IBoolType :: (HasBoolType expr arg builtin) => expr builtin
pattern IBoolType <- (getExpr accessBoolType -> Just ())
  where
    IBoolType = mkExpr accessBoolType ()

pattern IIndexType :: (HasIndexType expr arg builtin) => arg builtin -> expr builtin
...
pattern INatType :: (HasNatType expr arg builtin) => expr builtin
pattern INatType <- (getExpr accessNatType -> Just ())
  where
    INatType = mkExpr accessNatType ()

pattern IRatType :: (HasRatType expr arg builtin) => expr builtin
pattern IRatType <- (getExpr accessRatType -> Just ())
  where
    IRatType = mkExpr accessRatType ()

pattern IListType :: (HasListType expr arg builtin) => arg builtin -> expr builtin
pattern IListType tElem <- (getExpr accessListType -> Just (Op1Args tElem))
  where
    IListType tElem = mkExpr accessListType (Op1Args tElem)

pattern IVectorType :: (HasVectorType expr arg builtin) => arg builtin -> arg builtin -> expr builtin
...
pattern ITensorType :: (HasTensorType expr arg builtin) => arg builtin -> arg builtin -> expr builtin
...
pattern IBoolTensorLiteral :: (HasBuiltinConstructor expr arg, BuiltinHasBoolLiterals builtin) => BoolTensor -> expr builtin
pattern IBoolTensorLiteral n <- (getExpr accessBoolTensorLiteral -> Just n)
  where
    IBoolTensorLiteral n = mkExpr accessBoolTensorLiteral n

pattern IBoolLiteral :: (HasBoolExpr expr arg builtin) => Bool -> expr builtin
pattern IBoolLiteral n = IBoolTensorLiteral (ZeroDimTensor n)

pattern IIndexLiteral :: (HasIndexExpr expr arg builtin) => Int -> arg builtin -> expr builtin
...
pattern INatLiteral :: (HasNatExpr expr arg builtin) => Int -> expr builtin
pattern INatLiteral n <- (getExpr accessNatLiteral -> Just n)
  where
    INatLiteral n = mkExpr accessNatLiteral n

pattern INatTensor :: (HasNatExpr expr arg builtin) => Tensor Int -> expr builtin
pattern INatTensor n <- (getExpr accessNatTensorLiteral -> Just n)
  where
    INatTensor n = mkExpr accessNatTensorLiteral n

pattern IRatLiteral :: (HasRatExpr expr arg builtin) => Rational -> expr builtin
pattern IRatLiteral n = IRatTensor (ZeroDimTensor n)

pattern IRatTensor :: (HasRatExpr expr arg builtin) => Tensor Rational -> expr builtin
...
{-
pattern IDimNil ::
  (HasListExpr expr arg builtin, HasNatType expr arg builtin) =>
  expr builtin
...
pattern IVecLiteral :: (HasVectorExpr expr arg builtin) => arg builtin -> arg builtin -> [arg builtin] -> expr builtin

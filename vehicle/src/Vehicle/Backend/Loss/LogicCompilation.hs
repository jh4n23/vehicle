...
import Vehicle.Compile.Normalise.BuiltinForced (evalCompareRatTensorPointwise)
...
isLogicDecl typ = do
  normType <- runFreshNameBoundContextT $ forceThunk $ Unforced emptyBoundEnv typ
...
calculateLogicDirection declProv fields = do
  ...
  let args = TensorOp2Args (Forced IDimNil) trueValue falseValue
  result <- runFreshNameBoundContextT $ evalCompareRatTensorPointwise Le args
  case result of
    Evaluated (Forced (IBoolLiteral b)) -> return b
    _ -> throwError $ UnorderableDifferentiableLogic declProv (mkExpr accessCompareRatTensorPointwise (Le, args))

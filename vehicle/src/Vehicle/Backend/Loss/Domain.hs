...
compileBool value = logEntryAndExit value $ do
  ...
  case forcedValue of
    ...
    -- Blocked cases --
    -------------------
    VReduceAndTensor args -> unblockBoolValue (Forced $ mkExpr accessReduceAnd args)
    VReduceOrTensor args -> unblockBoolValue (Forced $ mkExpr accessReduceOr args)
    VBoolTensorAt args -> unblockBoolValue (Forced $ mkExpr accessAtTensor args)
    VBoolVectorAt args -> unblockBoolValue (Forced $ mkExpr accessAtVector args)
...
compileComparison (op, args) = do
  logCompilerSection2 MaxDetail "assertion compilation" $ do
    if op == Ne
      then singletonUnconstrainedPartition =<< toComparison (op, args)
      else do
